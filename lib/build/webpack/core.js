...
const { getPackageInfo } = require('../../utils/app');
...
module.exports = (
  ...
  const entryGetter = isUmd ? getUmdEntry({ target }) : getProjectEntry({ isDev, target, buildPath, splitPackages });
  ...
  const baseConfig = {
    ...
    output: {
      ...
      publicPath,
      filename: isUmd ? '[name]/index.js' : `static/scripts/[name].${chunkName}js`,
      sourceMapFilename: isUmd ? '[name]/index.map' : `static/scripts/[name].${chunkName}map`,
      chunkFilename: isUmd ? '[name]/[name].js' : `static/scripts/[name].${chunkName}js`,
      ...output
    },
  ...
  // 加载外部 plugins
  const otherPlugins = getPlugins ? getPlugins({ isDev, ...getPackageInfo() }) : null;
